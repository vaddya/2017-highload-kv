...
### Нагрузка
* Обязательно только `PUT` (без перезаписи) с `replicas=2/3` и `replicas=3/3`
* Обязательно только `GET` (на большом наборе ключей) с `replicas=2/3` и `replicas=3/3`
* Желательно смесь `PUT`/`GET` 50/50 (без перезаписи) с `replicas=2/3` и `replicas=3/3`
